...
# ==========================================
DATA_CONFIG = "C:/Users/mahal/Downloads/PythonProject/PythonProject/balloon2.v2i.yolov9/data.yaml"  # YAML file with dataset paths (train/valid/test)
MODEL_WEIGHTS = "balloon_yolov9.pt"  # File to save/load trained weights
# Load model with explicit image size
MODEL_CONFIG = YOLO("C:/Users/mahal/Downloads/PythonProject/PythonProject/balloon2.v2i.yolov9/yolov9.yaml")

# Run inference with matching image size on a sample image
results = MODEL_CONFIG.predict("C:/Users/mahal/Downloads/PythonProject/PythonProject/balloon2.v2i.yolov9/test/images/16_jpg.rf.daf113ebb3f2106fc29e9f7f4efdfe32.jpg", imgsz=[1080, 1920])
...
# HSV ranges for different colors (adjusted for purple from images)
HSV_RANGES = {
    "red": ([0, 120, 70], [10, 255, 255], [170, 120, 70], [180, 255, 255]),
    "blue": ([94, 80, 2], [126, 255, 255]),
    "purple": ([110, 30, 50], [170, 255, 255]),
    "green": ([40, 40, 40], [70, 255, 255])
}

# For RGB mode, example ranges (adjusted for purple from images)
RGB_RANGES = {
    "red": (np.array([100, 0, 0]), np.array([255, 100, 100])),
    "blue": (np.array([0, 0, 100]), np.array([100, 100, 255])),
    "purple": (np.array([75, 0, 75]), np.array([220, 150, 255])),
...
# ==========================================
# Utility: Create Trackbars for Debugging HSV Range
# ==========================================
def nothing(x):
    pass

cv2.namedWindow("Trackbars")
cv2.createTrackbar("LH", "Trackbars", 110, 180, nothing)
cv2.createTrackbar("LS", "Trackbars", 30, 255, nothing)
cv2.createTrackbar("LV", "Trackbars", 50, 255, nothing)
cv2.createTrackbar("UH", "Trackbars", 170, 180, nothing)
...
def color_filter_mask(frame):
    ...
    if COLOR_MODE == "HSV":
        ...
        # If debugging with trackbars, update range dynamically
        ...
        lower_dynamic = np.array([lh, ls, lv])
        upper_dynamic = np.array([uh, us, uv])
        if SELECTED_COLOR.lower() == "red":
            ...
            # Use the dynamic range from the trackbars for purple
            if SELECTED_COLOR.lower() == "purple":
                lower = lower_dynamic
                upper = upper_dynamic
            else:
            ...
            mask = None
        # For debugging: show the HSV image
    ...
    while True:
        ...
        if results.boxes is not None and len(results.boxes) > 0:
            ...
            if roi.size > 0:
                ...
                # Debug print for color ratio
                print("Color ratio:", color_pixels / total_pixels if total_pixels > 0 else 0)
                if total_pixels > 0 and (color_pixels / total_pixels) >= 0.5:
                    ...
                    if pos_data is not None:
                        ...
                        cv2.line(display_frame, (FRAME_WIDTH_PIXELS // 2, 0), (FRAME_WIDTH_PIXELS // 2, FRAME_HEIGHT_PIXELS), (255, 255, 255), 1)
                        ...
                        cv2.putText(display_frame, info_text, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                    else:
                        cv2.putText(display_frame, "Position error", (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                else:
                    cv2.putText(display_frame, "Color mismatch", (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
            else:
