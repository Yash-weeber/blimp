...
# Camera parameters
FOCAL_LENGTH_MM = 3.04  # Focal length in mm
SENSOR_WIDTH_MM = 3.68  # Sensor width of the camera in mm (Raspberry Pi Camera V2)
FRAME_WIDTH_PIXELS = 640  # Resolution width in pixels (adjust if different)
FOCAL_LENGTH_PIXELS = (FOCAL_LENGTH_MM / SENSOR_WIDTH_MM) * FRAME_WIDTH_PIXELS  # Focal length in pixels

# Balloon properties
KNOWN_WIDTH_CM = 5  # Approximate diameter of the balloon in cm (adjust as needed)

# HSV color ranges for balloons
BALLOON_COLORS = {
...
TRAIN_DIR = "E:/blimnptraiin/"
VALID_DIR = "E:/blimpval/"
MODEL_PATH = "shiny_balloon_model.h5"

# Function to train the shiny balloon detection model
...
# Function to calculate distance based on perceived width
def calculate_distance(known_width, focal_length, perceived_width):
    if perceived_width == 0:
        return None
    return (known_width * focal_length) / perceived_width

# Function to detect balloons of specific colors
def detect_balloons(frame, colors):
    ...
    for color_name, color_range in colors.items():
        ...
        for contour in contours:
            if cv2.contourArea(contour) > 50:  # Filter small objects
                # Get the bounding box
                x, y, w, h = cv2.boundingRect(contour)
                aspect_ratio = float(w) / h

                # Ensure the shape is roughly circular (aspect ratio ~1)
...
# Function to classify balloons as shiny or not shiny
...
def main():
    ...
    cap = cv2.VideoCapture(0)  # Default camera, or specify a video file for testing
    ...
    while True:
        ...
        for color_name, x, y, w, h, distance in detections:
            # Classify as shiny or not shiny
